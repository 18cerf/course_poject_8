...
const messagesTypes = {
    ...
    "game.gameOver": (message) => {
        updateGame(message);
        if (message.gameState === 'TIE') toastr.success(`Игра окончена, ничья!`);
...
const showWinner = (winner) => {
    toastr.success(`Победитель ${winner}!`);
...
const updateGame = (message) => {
    ...
    document.getElementById("player2").innerHTML = game.player2 || (game.winner ? '-' : 'Ожидание игрока 2...');
