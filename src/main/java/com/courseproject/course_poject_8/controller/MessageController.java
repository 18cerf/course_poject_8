...
import com.courseproject.course_poject_8.enumeration.GameState;
import com.courseproject.course_poject_8.manager.TicTacToeManager;
import com.courseproject.course_poject_8.model.TicTacToe;
import com.courseproject.course_poject_8.model.dto.JoinMessage;
import com.courseproject.course_poject_8.model.dto.PlayerMessage;
import com.courseproject.course_poject_8.model.dto.TicTacToeMessage;
...
public class MessageController {
    ...
    public Object joinGame(@Payload JoinMessage message, SimpMessageHeaderAccessor headerAccessor) {
        ...
        if (game == null) {
            ...
            errorMessage.setType("Ошибка");
            errorMessage.setContent("Невозможно войти в игру. Возможно, игра уже заполнена или произошла внутренняя ошибка.");
            return errorMessage;
        }
    ...
    public void makeMove(@Payload TicTacToeMessage message) {
        ...
        if (game == null || game.isGameOver()) {
            TicTacToeMessage errorMessage = new TicTacToeMessage();
            errorMessage.setType("Ошибка");
            errorMessage.setContent("Игра не найдена или уже завершена.");
        ...
        if (game.getGameState().equals(GameState.WAITING_FOR_PLAYER)) {
            ...
            errorMessage.setContent("Игра ожидает подключения другого игрока.");
            this.messagingTemplate.convertAndSend("/topic/game." + gameId, errorMessage);
            return;
        }
        ...
        if (game.getTurn().equals(player)) {
            ...
            if (game.isGameOver()) {
                ...
                gameInfoRepository.save(new GameInfo(game.getWinner()));
